...
class C_Status:
    ...
    def setContour(self, contours):
        if self.current_contours is None:
            ...
            if self.similarity(contours):
                if self.Reset_counter > 500: # Re-detect
                    self.current_contours = contours
                    self.MAX_LEN = len(contours)
                    self.Reset_counter = 0
            else: # Time's up, update or similarity not true
                if self.Reset_counter > 100: # Re-detect in 5s (quickly)
                    self.current_contours = contours
                    self.MAX_LEN = len(contours)
                    self.Reset_counter = 0
    ...
    def drawImg(self, x0, y0, x1, y1, color='green', box_label:str=None):
        ...
        elif _C == 'white': COLOR_ = (255, 255, 255)
...
def Human_Detection(cur_status: C_Status, Poly, human_model):
    ...
    #     cur_status.drawImg(p['x0'], p['y0'], p['x1'], p['y1'], color='white', box_label=None)
...
def VPS_Detection(cur_status: C_Status, vps_model):
    ...
    for _, p in vps_results.iterrows():
        cur_status.drawImg(p['x0'], p['y0'], p['x1'], p['y1'], color='blue', box_label=None)
    for idx, human_p in humen.iterrows():
        ...
        for _, vps_p in vps_results.iterrows():
            ...
            if get_iou(human_p, vps_p) > 0.8:
                ...
                break
        if inside_Flag:
            cur_status.insert_point(humen.iloc[[idx]], mode=0) # Not Violate Yes
        else:
...
def Helmet_Detection(cur_status: C_Status, helmet_model):
    ...
    if len(helmet_results.index) == 0 and len(humen.index) > 0:
        ...
        return
    elif len(helmet_results) < len(humen):
        cur_status.setFlag(True) # Human more than helmet in Contours
    for _, p in helmet_results.iterrows():
        cur_status.drawImg(p['x0'], p['y0'], p['x1'], p['y1'], color='green', box_label=None)
    for idx, human_p in humen.iterrows():
        inside_Flag = False
        ...
        if inside_Flag:
            ...
            cur_status.drawImg(human_p['x0'], human_p['y0'], human_p['x1'], human_p['y1'], color='any', box_label=None)
        else:
    ...
    if len(cur_status.qualified_list.index) == 0:
        ...
        for v_idx, vio_p in cur_status.violate_list.iterrows():
            for q_idx, qua_p in cur_status.qualified_list.iterrows():
                # Same human from two adjacent frames (t, t+1), maybe not qualified anymore
                if get_iou(vio_p, qua_p) > 0.85:
                    u_tmp = qua_p['update'] + 1
                    cur_status.insert_point(vio_p, mode=2, u=u_tmp)
                    remove_list.append(q_idx)
                    # results[v_idx] = False
                    break
        # quialified double check
        # for h_idx, human_p in cur_status.human_list.iterrows():
        #     for q_idx, qua_p in cur_status.qualified_list.iterrows():
        #         # Same human
        #         if get_iou(human_p, qua_p) > 0.9:
        #             if qua_p['update'] - 2 < 0: qua_p['update'] = 0
        #             else: qua_p['update'] -= 2
        #             break
...
def Stream(models, input_frame, id, video):
    with torch.no_grad():
        # DATE_TIME = str(datetime.now().strftime('%Y_%m_%d_%H_%M_%S')) # Get Current Date Time
        Image_frame = input_frame # Get Image Frame from Camera as type<numpy array>
        C_S.setImg(Image_frame) # Load Image
        Step_1 = Cone_Detection(C_S, cone_model=models[0])
        if not Step_1: # No polygon detected, next Image
            video.write(C_S.draw_by_self(mode=1))
            # cv2.imwrite('results/uncertain_3/' + str(id) + '.jpg', C_S.draw_by_self(mode=1))
            # print('Phase 1')
            # C_S.draw_by_self()
            return
        poly_path = mpltPath.Path(C_S.get_Contours())
        Step_2 = Human_Detection(C_S, poly_path, human_model=models[1])
        if not Step_2: # No human detected or not in polygon
            ...
            # C_S.draw_by_self()
            return
        VPS_Detection(C_S, vps_model=models[2])
        if C_S.get_human(mode=0) is not None:
            Helmet_Detection(C_S, helmet_model=models[3])
        ...
        if C_S.get_CStatus() > 20:
            ...
            if violate_list is not None and len(qua_list) > 0:
                for v_idx, p in violate_list.iterrows():
                    ...
                    # else: C_S.drawImg(p['x0'], p['y0'], p['x1'], p['y1'], color='any', box_label=None)
                # Notification (Output suspicious image to local and alert)
                # if sum(qua_list):
        video.write(C_S.get_Img(select=1))
        cv2.imwrite('results/uncertain_3/' + str(id) + '.jpg', C_S.get_Img(select=1))
        C_S.reset_status()
        return
...
if __name__ == '__main__':
    ...
    frames = range(1057) # 450, 749
    # DATE_TIME = str(datetime.now().strftime('%Y_%m_%d_%H_%M_%S')) # Get Current Date Time
    out = cv2.VideoWriter('results/1216frame.mp4', cv2.VideoWriter_fourcc(*'DIVX'), 10, (1920, 1080))
    # out = None
    t_sum = 0
    t_max = 0
    for i in tqdm(frames):
        # input_frame = cv2.imread(os.path.join('1028_frame', str(i) + '.jpg'))
        # input_frame = cv2.imread(os.path.join('0929_frame', str(i) + '.jpg'))
        start_time = time()
        input_frame = cv2.imread(os.path.join('1216_frame', str(i) + '.jpg'))
        # input_frame = cv2.imread(os.path.join('yolov5-master', 'data', 'images', 'bus.jpg'))
        Stream(models, input_frame, i, out)
        end_time = time()
        t_max = (end_time - start_time) if (end_time - start_time) > t_max else t_max
        t_sum += end_time - start_time
    # out.release()
    print(f'Average FPS: {int(len(frames) / t_sum)}')
    print('Maximum exe time %.2f second'%t_max)
