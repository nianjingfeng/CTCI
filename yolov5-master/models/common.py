    ...
    def forward(self, ims, size=640, augment=False, profile=False):
        ...
        with dt[0]:
            ...
            if isinstance(ims, torch.Tensor):  # torch
                # print('Tensor detect without autocast')
            ...
            # x = np.ascontiguousarray(np.array(im).transpose((2,0,1))[::-1])
            x = np.ascontiguousarray(np.array(x).transpose((0, 3, 1, 2)))  # stack and BHWC to BCHW
            # x = torch.from_numpy(x).to(p.device).type_as(p) / 255  # uint8 to fp16/32
            x = torch.from_numpy(x).to(p.device)
            x = x.half() if self.model.fp16 else x.float()
            x /= 255
            # print('Data augment done')

        with amp.autocast(autocast):
        # print('Inference without autocast')
            # Inference
            with dt[1]:
                print('Forward')
...
class Detections:
    ...
    def _run(self, pprint=False, show=False, save=False, crop=False, render=False, labels=True, save_dir=Path(''), get_img = False, without_conf = False):
        s, crops = '', []
