...
def predict_human(human_model, src_img):
    human_result = human_model(src_img, size=1080)
...
def predict_vps(vps_model, src_img):
    vps_result = vps_model(src_img, size=1080)
...
def cone_determine(cur_cone, new_cone):
    if len(new_cone) > MAX_CONTOURS: # max_len
        cone_detect_counter = 0
        MAX_CONTOURS = len(new_cone)
        return new_cone
    elif cone_similarity(new_cone, cur_cone): # Occlude but same
        cone_detect_counter += 1
        if cone_detect_counter > COUNTER: # Re-detect
            cone_detect_counter = 0
            return new_cone
        else: return cur_cone
    elif not cone_similarity(new_cone, cur_cone): # Move cones
        cone_detect_counter = 0
        return new_cone
    else: # Times up, update
        cone_detect_counter += 1
        if cone_detect_counter > COUNTER: # Re-detect
            cone_detect_counter = 0
            return new_cone
...
def cone_detection(model, img):
    ...
    result = model(img, size=1080) # print(), show(), save(), crop(), render(), pandas(), tolist(), __len__()
        ...
        while True:
            ...
            if q.empty() != True:
                ...
                if previous_contours is None and current_contours is None:
                    ...
                    current_contours = cone_determine(contours, previous_contours)
                ...
                output_img = draw_helmet(helmet_model, output_img)
...
def Display():
    out = cv2.VideoWriter('demo.mp4', cv2.VideoWriter_fourcc(*'DIVX'), 10, (1920, 1080))
    while True:
        if q_yolo.empty() != True:
            ...
            try:
                ...
                cv2.resizeWindow("Stream",300,700)
                '''
...
if __name__ == "__main__":
    # cone_model = torch.hub.load(('./yolov5-master'), 'custom', path = './Models/cone_model_0706_new.pt', source='local')
    ...
    # Predict vps
    # vps_model = torch.hub.load(('./yolov5-master'), 'custom', path = './Models/vps_model_0708.pt', source='local')
    vps_model = torch.hub.load(('./yolov5-master'), 'custom', path = './Models/vps_model_0708.pt', source='local')
    vps_model.eval()
    vps_model.cuda()
    vps_model.conf = 0.7
